...
def render_chart_page():
    ...
    with cols[4]: st.metric("Total days receive", day_rcv)
    with cols[5]: st.metric("Total days ship", day_so)
    with cols[6]: st.metric("Total items receive", item_rcv)
    with cols[7]: st.metric("Total item ship", item_so)
    with cols[8]: st.metric("Amount Receive/Ship", f"{amount_rcv:.0f}/{amount_so:.0f}")
